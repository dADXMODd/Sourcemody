...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jepiq.tgbot.send_file(
                ...
                buttons=[(Button.url("سورس زد إي", "https://t.me/UI_OS"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**᯽︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت زد إي تمت بنجاح**"
            )
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/ZE.JPEG")
        _, groupid = await create_supergroup(
            "مجموعة أشعارات زد إي ", jepiq, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/ZE2.JPEG")
        _, groupid = await create_supergroup(
