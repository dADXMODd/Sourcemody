...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "الحمد لله دائماً وابداً 🎀 𝑆𝐻 : @UI_9U"
)
...
async def _(event):
    ...
    if replied_user.id == 6686509758:
        return await edit_delete(event, "**لا تحاول تنتحل المطورين!**")
    if replied_user.id == 6686509758:
        return await edit_delete(event, "**لا تحاول تنتحل المطورين!**")
    if replied_user.id == 6686509758:
        return await edit_delete(event, "**لا تحاول تنتحل المطورين!**")
...
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        if len(messageo) != 8:
            return await edit_delete(event, "لا تغير الرسالة @UI_9U")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
